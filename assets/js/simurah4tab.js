...
// =========================================================
    ...
    function fetchDataVerifikasi() {
        ...
        if (!isPinValid) {
            statusDivVerif.className = 'status-error-2';
            statusDivVerif.innerText = 'PIN tidak valid. Silakan verifikasi PIN untuk memuat data.';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleView(targetId) {
            ...
            } else if (targetId !== 'input-absen') {
                 // Jika PIN tidak valid, dan bukan tab input, tampilkan pesan error
                const loadingStatusEl = document.getElementById(`loading-status-${targetId.replace('rekap-', 'rekap-')}`);
                if(loadingStatusEl) {
                     updateStatus(`loading-status-${targetId.replace('rekap-', 'rekap-')}`, "PIN tidak valid. Silakan verifikasi PIN untuk memuat data.", 'error');
                }
            }
